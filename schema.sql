...
-- Bookings_Tickets Table (Many-to-Many)
...
-- automate addition of available seats when a new show is added
...
CREATE COVERING INDEX show_seats ON Shows(available_seats);
CREATE COVERING INDEX audi_seat ON Seats(audi_id);
-- Optimize on updating ticket status
CREATE COVERING INDEX update_booked ON Tickets(booked);
-- Optimize box office calculations
CREATE COVERING INDEX ticket_price ON Tickets(price);